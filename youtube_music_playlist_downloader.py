...
def update_file_order(
    playlist_name, song_file_info, track_num, config: dict, missing_video: bool
):
    ...
    if song_file_info.track_num != track_num and config["include_metadata"]["track"]:
        if missing_video:
            print(
                f"Reordering '{song_file_info.name}' from position {song_file_info.track_num} to {track_num} due to missing video link..."
            )
        else:
            print(
                f"Reordering '{song_file_info.name}' from position {song_file_info.track_num} to {track_num}..."
            )
    ...
    if song_file_info.file_path != file_path:
        if song_file_info.track_num == track_num:
            ...
            print(
                f"Renaming incorrect file name from '{song_file_info.file_name}' to '{file_name}'"
            )
    ...
    try:
        ...
        if force_update:
            try:
                ...
                force_update_file_name = get_song_info_ytdl(
                    track_num, config
                ).prepare_filename(info_dict_with_audio_ext)
            except Exception as e:
                raise Exception(
                    f"Failed to get information for updated file name - {e}"
                )
    ...
    if regenerate_metadata or force_update or not valid_metadata(config, metadata_dict):
        ...
        try:
            ...
            if not metadata_dict["APIC:Front cover"] and include_metadata["cover"]:
                ...
                top = half_height - min_offset
                bottom = half_height + min_offset
                img_data = convert_image_type(
                    img.crop([left, top, right, bottom]), config["image_format"]
                )
                tags.add(
                    APIC(
                        3, f"image/{config['image_format']}", 3, "Front cover", img_data
                    )
                )
            ...
            if not metadata_dict["TDRC"] and include_metadata["date"]:
                tags.add(
                    TDRC(
                        encoding=3,
                        text=time.strftime(
                            "%Y-%m-%d", time.strptime(upload_date, "%Y%m%d")
                        ),
                    )
                )
            ...
            if include_metadata["lyrics"] and (
                not metadata_dict["SYLT"] or not metadata_dict["USLT"]
            ):
                synced_lyrics = []
                unsynced_lyrics = []
                lang = "en"
                ...
                if subtitles and requested_subtitles and len(subtitles) > 0:
                    ...
                    try:
                        if len(lyrics_langs) == 0:
                            ...
                            print(
                                f"Selecting first available language for lyrics: {lang}"
                            )
                        else:
                            lyrics_found = False
                            for lyrics_lang in lyrics_langs:
                                for requested_lang in requested_subtitles.keys():
                                    ...
                                    if re.match(
                                        r"^" + lyrics_lang + r"$", requested_lang
                                    ):
                                        subtitles_url = get_subtitles_url(
                                            subtitles, requested_lang
                                        )
                                        lang = requested_lang
                            ...
                            if subtitles_url is None:
                                available_languages_str = str(
                                    list(requested_subtitles.keys())
                                )
                                print(
                                    f"Lyrics unavailable for selected languages. Available languages: {available_languages_str}"
                                )
                    ...
                    if subtitles_url is not None:
                        try:
                            content = json.loads(
                                requests.get(subtitles_url, stream=True).text
                            )
                            ...
                            for event in content["events"]:
                                ...
                                ) < 1000 and line.strip() in last_lines:
                ...
                tags.add(
                    SYLT(encoding=3, lang=lang, format=2, type=1, text=synced_lyrics)
                )
...
def download_song_and_update(
    video_info, playlist, link, playlist_name, track_num, config: dict
):
    file_path = None
    try:
        ...
        generate_metadata(
            file_path, link, track_num, playlist["title"], config, False, False
        )
    except Exception as e:
...
def update_song(
    ...
    error_message = []
    try:
        force_update_file_name = generate_metadata(
            file_path,
            link,
            track_num,
            playlist_name,
        ...
        if force_update:
            ...
            if file_path != force_update_file_path:
                ...
                print(
                    f"Renaming incorrect file name from '{Path(file_path).stem}' to '{Path(force_update_file_path).stem}'"
                )
                os.rename(file_path, force_update_file_path)
    except Exception as e:
        error_message.append(
            f"Unable to update metadata for #{track_num} '{link}': {e}"
        )
    ...
    if video_info["channel_id"] is None or video_unavailable:
        if len(error_message) == 0:
            ...
            error_message.append(
                f"Unable to fully update metadata for #{track_num} '{link}'"
            )
        error_text = f"The previous song '{song_file_info.name}' is unavailable but a local copy exists"
        if (
            not video_unavailable
            and video_info["title"] is not None
            and video_info["title"] != ""
        ):
            # Video title indicates availability of video such as '[Private Video]'
...
def get_song_file_info(playlist_name, song_file_name) -> SongFileInfo:
    ...
    return SongFileInfo(
        song_video_id, song_name, song_file_name, song_file_path, song_track_num
    )
    ...
    for file_name in os.listdir(playlist_name):
        ...
        if song_file_info.video_id in song_file_infos:
            ...
            if song_file_info.video_id not in duplicate_files:
                duplicate_files[song_file_info.video_id] = [
                    song_file_infos[song_file_info.video_id].file_name
                ]
...
def setup_config(config: dict):
    ...
    new_config["overrides"] = {"EXAMPLE_VIDEO_ID_HERE": config_copy}
...
def generate_playlist(
    ...
    while True:
        ...
        if update:
            ...
            if (
                current_playlist_name is not None
                and current_playlist_name != adjusted_playlist_name
            ):
                ...
                print(
                    f"Renaming playlist from '{current_playlist_name}' to '{adjusted_playlist_name}'..."
                )
    ...
    song_file_infos = get_song_file_infos(
        playlist_name
        ...
        if config["retain_missing_order"]:
            ...
            if not found:
                ...
                playlist_entries.insert(
                    index, {"id": video_id, "channel_id": None, "title": None}
                )
    ...
    for i, video_info in enumerate(playlist_entries):
        ...
        if track_num_to_update is not None and (
            song_file_info is None or song_file_info.track_num != track_num_to_update
        ):
            continue
            ...
            if song_file_info is not None:
                ...
                try:
                    ...
                    force_update_file_name = generate_metadata(
                        file_path,
                        link,
                        song_file_info.track_num,
                        playlist["title"],
                        config,
                        regenerate_metadata,
                        True,
                    )
                    force_update_file_path = os.path.join(
                        playlist_name, force_update_file_name
                    )
                    if file_path != force_update_file_path:
                        # Track name needs updating to proper format
                        print(
                ...
                print(
                    f"Unable to update metadata for '{link}': This song has not been downloaded yet, please update the playlist first"
                )
        ...
        if song_file_info is None:
            ...
            print(
                f"Downloading '{link}'... ({track_num}/{len(playlist_entries) - skipped_videos})"
            )
            ...
            if base_config["use_threading"]:
                ...
                error_message, _ = download_song_and_update(
                    video_info, playlist, link, playlist_name, track_num, config
                )
            ...
            print(
                f"Skipped downloading '{link}' ({track_num}/{len(playlist_entries) - skipped_videos})"
            )
            ...
            if base_config["use_threading"]:
                ...
                file_path = update_file_order(
                    playlist_name, song_file_info, track_num, config, False
                )
    ...
    if base_config["use_threading"]:
        ...
        skipped_track_nums = [
            track_num
            for (error_message, track_num) in results
            if error_message is not None
        ]
        temp_song_file_infos = get_song_file_infos(
            playlist_name
        )  # May raise exception for duplicate songs
        for i, video_info in enumerate(playlist_entries):
            if video_info is None:
            ...
            if temp_song_file_info is not None:
                ...
                config = get_override_config(video_id, base_config)
                file_path = update_file_order(
                    playlist_name, temp_song_file_info, track_num, config, False
                )

    # Song not found for single song update
    if track_num_to_update is not None:
        print(
            f"Unable to update metadata for song #{track_num_to_update}: This song could not be found or is unavailable, please update the playlist first"
        )
        return
    ...
    for video_id in song_file_infos.keys():
        if video_id not in updated_video_ids:
            ...
            file_path = update_file_order(
                playlist_name, song_file_info, track_num, config, True
            )
            track_num += 1
    ...
    for playlist_name in next(os.walk(directory))[1]:
        ...
        if os.path.exists(config_file):
            ...
            try:
                ...
                print(
                    f"[ERROR] Playlist URL in config file '{config_file}' is in an invalid format. Please fix or remove the config file."
                )
                continue
            ...
            if playlist_id in playlists_name_dict:
                ...
                if playlist_id not in duplicate_playlists:
                    duplicate_playlists[playlist_id] = [
                        playlists_name_dict[playlist_id]
                    ]
            ...
            playlist_data = {
                ...
                "last_updated": time.strftime(
                    "%x %X", time.localtime(os.path.getmtime(config_file))
                ),
            }
            ...
            exception_strings.append(
                "\n".join(
                    [
                        ...
                        "\n".join(
                            ["- " + playlist_name for playlist_name in playlist_names]
                        ),
                    ]
                )
            )
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            options = [OPTION_DOWNLOAD, OPTION_GENERATE, OPTION_CHANGE, OPTION_EXIT]
            ...
            if single_playlist:
                ...
                while True:
                    try:
                        ...
                        quit_enabled = True
                        input(
                            "Press 'Enter' to continue after resolving this conflict or close this window to finish."
                        )
                        continue
                    except:
                        print(
                ...
                selected_option = options[
                    get_index_option_response("Select an option", len(options))
                ]
            ...
            if selected_option == OPTION_DOWNLOAD:
                ...
                for playlist_data in playlists_data:
                    try:
                        ...
                        if get_url_parameter(
                            existing_config["url"], "list"
                        ) == get_url_parameter(config["url"], "list"):
                            ...
                            update_existing = get_bool_option_response(
                                "This playlist is already downloaded. Update playlist?",
                                default=True,
                            )
                            if not update_existing:
                                ...
                                quit_enabled = True
                                input(
                                    "Press 'Enter' to return to main menu or close this window to finish."
                                )
                            else:
                ...
                if not already_downloaded and not update_existing:
                    config["reverse_playlist"] = get_bool_option_response(
                        "Reverse playlist?", default=False
                    )
                    config["use_title"] = get_bool_option_response(
                        "Use title instead of track name?", default=True
                    )
                    config["use_uploader"] = get_bool_option_response(
                        "Use uploader instead of artist?", default=True
                    )
                    config["use_playlist_name"] = get_bool_option_response(
                        "Use playlist name for album?", default=True
                    )
                    ...
                    quit_enabled = True
                    input(
                        "Finished downloading. Press 'Enter' to return to main menu or close this window to finish."
                    )

            if (
                selected_option == OPTION_UPDATE
                or selected_option == OPTION_SONG
                or update_existing
            ):
                ...
                if update_existing:
                    ...
                    for i, playlist_data in enumerate(playlists_data):
                        playlists_list.append(
                            f"{i + 1}. {playlist_data['playlist_name']} (Last Updated: {playlist_data['last_updated']})"
                        )
                    print("\n" + "\n".join(playlists_list) + "\n")
                ...
                if selected_option == OPTION_SONG:
                    track_num_to_update = get_numeric_option_response(
                        "Enter a song track number to update"
                    )
                ...
                quit_enabled = True
                input(
                    "Finished updating. Press 'Enter' to return to main menu or close this window to finish."
                )
            ...
            if selected_option == OPTION_MODIFY or modify_existing:
                ...
                modify_settings = get_bool_option_response(
                    "Change playlist settings?", default=False
                )
                quit_enabled = False
                ...
                if modify_settings:
                    ...
                    if (
                        config["use_title"] != last_use_title
                        or config["use_uploader"] != last_use_uploader
                        or config["use_playlist_name"] != last_use_playlist_name
                    ):
                        regenerate_metadata = True
                elif single_playlist:
                    update_single_song = get_bool_option_response(
                        "Update a single song?", default=False
                    )
                ...
                if not update_single_song:
                    force_update = get_bool_option_response(
                        "Force update all names and metadata?", default=False
                    )
            ...
            if selected_option == OPTION_GENERATE:
                ...
                config["url"] = input(
                    "Please enter the URL of the playlist to generate config for: "
                )
                ...
                for playlist_data in playlists_data:
                    try:
                        ...
                        if get_url_parameter(
                            ...
                            print(
                                f"Playlist '{playlist_data['playlist_name']}' is already downloaded."
                            )
                            quit_enabled = True
                            input(
                                "Press 'Enter' to return to main menu or close this window to finish."
                ...
                if not already_downloaded:
                    ...
                    quit_enabled = True
                    input(
                        "Finished generating default config. Press 'Enter' to return to main menu or close this window to finish."
                    )

            if selected_option == OPTION_CHANGE:
                # Change current working directory
                target_path = input(
                    "Enter path of target playlists folder to change to: "
                )
